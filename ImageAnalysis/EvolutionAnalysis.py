...
#                    self.individual_cell_dic_bef[str(RegionLoopNumber)] = self.RawRegionImg_for_contour # put each cell region into a dictionary
#                    self.individual_cell_dic_aft[str(RegionLoopNumber)] = self.regionimage_after_for_contour
#                    if RegionLoopNumber == 2:
#                        plt.figure()
#                        plt.imshow(self.RawRegionImg, cmap = plt.cm.gray) 
#                        plt.show() 

                    #print(np.max(self.RawRegionImg))
...
#                                print(EachsubcellregionCoords.shape)
                    ...
                    contour_mask_bef = imageanalysistoolbox_instacne.contour(self.filled_mask_bef, self.RawRegionImg_for_contour.copy(), contour_thres)
...
#                                print(EachsubcellregionCoords.shape)
                ...
                # after here self.intensityimage_intensity is changed from contour labeled with number 5 to binary image
                contour_mask_of_cell = imageanalysistoolbox_instacne.inwarddilationmask(contour_mask_bef ,self.filled_mask_convolve2d, contour_dilationparameter)

                contour_origin_image_intensity = self.RawRegionImg*contour_mask_of_cell # Intensity image of cell contour
                
                # Get the skeleton of the contour
#                contour_skeleton = skeletonize(contour_mask_of_cell)#, method='lee'                
                thinned_partial = thin(contour_mask_bef, max_iter=25)
...
#                contour_skeleton_pruned = opening(contour_skeleton, square(int(cell_prune_opening_factor)))
                ...
                ax2.imshow(self.filled_mask_convolve2d, cmap = plt.cm.gray)
...
if __name__ == "__main__":
    ...
    PMT_image = imread(r'D:\XinMeng\imageCollection\Round4_Coords14_R4500C1500_PMT_0Zmax.tif', as_gray=True)[:, 50:550]
    ...
    CellPropDictEachRound = CellContour_instance.get_Skeletonized_contour(PMT_image, RegionProposalMask, smallest_size=300, contour_thres=0.001, 
                                                                                       contour_dilationparameter=15, cell_region_opening_factor=1, cell_region_closing_factor=2)
