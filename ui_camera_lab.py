...
from camera_backend import Camera
...
class CameraROI(QWidget):
    ...
    def spin_value_changed(self):
        ...
        if self.x_size.value() != self.roi_x_size or self.y_size.value() !=\
        self.roi_y_size:
        ...
        if self.center_roiButton.isChecked():
            ...
            if self.x_position.value() != self.roi_x or self.y_position.value() \
            != self.roi_y:
...
if __name__ == "__main__":
    def run_app():
        '''
        Run the total application (Camera UI)
        '''
    ...
    def check_qinput():
        '''
        For testing the timed popup window standalone. 
        '''
